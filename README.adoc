...
It will respond with a JSON representation of a Message, as the following listing shows:
...
You can customize the Message with an optional `name` parameter in the query string, as
the following listing shows:

====
[source,text]
----
http://localhost:8080/Message?name=User
----
====
...
The service will handle `GET` requests for `/Message`, optionally with a `name` parameter
in the query string. The `GET` request should return a `200 OK` response with JSON in the
body that represents a Message. It should resemble the following output:
...
The `id` field is a unique identifier for the Message, and `content` is the textual
representation of the Message.

To model the Message representation, create a resource representation class. To do so,
provide a Java record class for the `id` and `content` data, as the following listing (from
`src/main/java/com/example/restservice/Message.java`) shows:

[source,java]
----
include::complete/src/main/java/com/example/restservice/Message.java[]
----

====
NOTE: This application uses the https://github.com/FasterXML/jackson[Jackson JSON] library to
automatically marshal instances of type `Message` into JSON. Jackson is included by default by the web starter.
====
...
annotation, and the `MessageController` shown in the following listing (from
`src/main/java/com/example/restservice/MessageController.java`) handles `GET` requests
for `/Message` by returning a new instance of the `Message` class:

====
[source,java]
----
include::complete/src/main/java/com/example/restservice/MessageController.java[]
----
====
...
The `@GetMapping` annotation ensures that HTTP GET requests to `/Message` are mapped to the `Message()` method.
...
parameter of the `Message()` method. If the `name` parameter is absent in the request,
the `defaultValue` of `World` is used.

The implementation of the method body creates and returns a new `Message` object with
`id` and `content` attributes based on the next value from the `counter` and formats the
given `name` by using the Message `template`.
...
relying on a view technology to perform server-side rendering of the Message data to
HTML, this RESTful web service controller populates and returns a `Message` object. The
...
The `Message` object must be converted to JSON. Thanks to Spring's HTTP message converter
...
is automatically chosen to convert the `Message` instance to JSON.
...
Now that the service is up, visit `http://localhost:8080/Message`, where you should see:
...
`http://localhost:8080/Message?name=User`. Notice how the value of the `content`
...
This change demonstrates that the `@RequestParam` arrangement in `MessageController` is
...
working against the same `MessageController` instance across multiple requests and that
